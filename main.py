...
# High-level planner: scene breakdown + object selection (structured output)
planner_llm_raw = ChatOpenAI(
    model="gpt-4.1-mini",
    temperature=0.2,
)

planner_llm = planner_llm_raw.with_structured_output(ScenePlan)

# Code generator / fixer: produces & repairs Manim code
...
def generate_manim_video_from_prompt(prompt: str, max_attempts: int = 2) -> ManimState:
